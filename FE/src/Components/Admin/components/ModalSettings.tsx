...
export const Modal = ({ closeModal, onSubmit, defaultValue }) => {
  ...
  const validateForm = () => {
    if (formState.id && formState.value) {
      ...
      let errorFields: string[] = []; // Explicitly declare the type as string[]
      for (const [key, value] of Object.entries(formState)) {
        ...
        if (!value) {
          ...
          if (key === "id") {
            const valueAsString = value as string; // Assert value as a string
            if (!(Object.keys(dataJSON).includes(valueAsString) || valueAsString === "ALL")) {
              errorFields.push("INVALID_ID_" + valueAsString);
            }
          }
        }
      }
